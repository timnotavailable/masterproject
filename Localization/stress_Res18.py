## model and label should be double! notice
import hydra
import hydra.utils
...
@hydra.main(config_path="config/train",config_name="stress_Res18.yaml")   
def train_model(cfg:DictConfig):
    ...
    classifier=Classifier(cfg.model).to(device)
    ## tensorboard usage
        ...
        for i,data_batch in enumerate(tqdm(train_dataloader),1):
            ...
            if global_step % cfg.training_settings.checkpoint_interval==0:
                ...
                with open(str(validation_csv_path),'a') as fd:
                    ...
                    fd.write(',' + str(np.mean(statistics['Sensitivity'])))
...
#                 print("logit_batch dtype is ", logit_batch.dtype)
                val_loss+=loss_function(logit_batch,label_batch.double()).item()
