...
## custom dataset,  import every classifiers, with multi and without
from utils import MPIDataset,save_checkpoint, load_checkpoint,Evaluator,Multi_Evaluator 
...
@hydra.main(config_path="config",config_name="test_mmd.yaml")
def test_model(cfg:DictConfig):
    ...
    for i in classifiers_list:
        ...
        pretrained_path=Path(model_config.pretrained_path)/model_config.pretrained_name
        classifier=Classifier(pretrained_path,model_config.alpha).to(device)  
        ...
        statistics,tabular_report=multi_evaluator.evaluate(classifier,MPI_dataloader)
        print('MPI test:')
        print(statistics)
        ...
        print('recall is : {}'.format(np.mean(statistics['Sensitivity'])))
